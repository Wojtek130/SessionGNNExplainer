...
class SessionGraph(Module):
    ...
    def compute_scores(self, hidden, mask, x, alias_inputs, items, modify_global_embedding = False):
        ...
        if modify_global_embedding:
            product_ids = torch.gather(items, 1, alias_inputs)  # (batch_size, max_session_length)
            x_values = x[product_ids] # (batch_size, max_session_length)
            alpha = alpha * x_values
        ...
        a = a + target  # b,n,d
        scores = torch.matmul(a, b.transpose(1, 0)).squeeze()
        probs = F.log_softmax(scores, dim=1) # n,n
        return probs
